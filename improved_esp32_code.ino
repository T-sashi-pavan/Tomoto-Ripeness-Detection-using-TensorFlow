...
// Your Flask server if you want ESP to POST images (optional)
String serverName = "http://192.168.29.160:5000/predict";

httpd_handle_t stream_httpd = NULL;
camera_config_t config;

unsigned long lastPredictionTime = 0;
const unsigned long predictionInterval = 10000; // 10s
...
void setup() {
  ...
  config.fb_count = 1;                // IMPORTANT — reduce RAM usage
  // ===========================
  ...
  // small sensor tweaks (kept conservative)
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  // Serve frames continuously — keep small delay to avoid overloading
  while (true) {
    ...
    if (res != ESP_OK) break;
    delay(80); // ~12 FPS – reduce stress on camera
  }
  return res;
}
...
// Optional: POST images to server periodically (keeps small, infrequent uploads)
